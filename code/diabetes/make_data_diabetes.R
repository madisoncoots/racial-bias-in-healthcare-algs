...
table_write_path <- directory_path %>% str_remove("code/diabetes") %>% str_c("data/raw/")
...
write_csv(raw_weight_hist_all, file = paste(table_write_path, "raw_weight_hist_all.csv", sep = ""))
...
data <- raw_demographics_all %>%
  ...
  select(seqn, wtmec8yr, ridageyr, race, gender, bmxbmi, whd140, bmxwt, bmxht,
         bmxwaist, relatives_had_diabetes, felt_depressed, income, 
         health_insurance, food_security, diabetes)


saveRDS(data, file = paste(data_object_write_path, "diabetes_data.rds", sep = ""))
